...
public class DialogflowController {
    ...
    public void dialogflow(@RequestBody DialogFlowRequest dialogFlowRequest) throws Exception {
       ...
       String salida=dialogFlowRequest.getQueryResult().getQueryText();
       Parameters parametros=dialogFlowRequest.getQueryResult().getParameters();
       requestPrueba(request, parametros,salida);
    }

    public void requestPrueba(String request, Parameters parametros,String salida) throws Exception {
        switch (request){
            ...
            case "Su programa se ha entregado a su correo anteriormente solicitado":
                storageService.creacionZip(parametros.getAny(), parametros.getGmail(), parametros.getLenguajes(),salida);
                break;
        }
    }
}
